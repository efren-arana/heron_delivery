...
class ItemServiceFirebaseImpl implements AbstItemService {
  ...
  final String path = "/shops/fn6npNA8Xm4TvuPKB2VE/items";
  ...
  Stream<List<ItemCategoryModel>> streamOfCategories() {
    ...
    return _db.collection('product_category')
        .where('status', isEqualTo: "A")
        .snapshots()
        .map<List<ItemCategoryModel>>((event) {
      if (event.docs.isNotEmpty) {
        categories = event.docs
            .map((snapshot) => ItemCategoryModel.fromJson(snapshot.data(), snapshot.id))
            .toList();
      }
      return categories;
    });
